<template>
  <Row type="flex" :gutter="18">
    <Col :span=19>
    <Panel shadow>
      <div slot="title">{{$t('m.Quiz_List')}}</div>
      <div slot="extra">
        <ul class="filter">
          <li>
  ...
  export default {
        ...
        quizTableColumns: [
          {
            ...
            render: (h, params) => {
              return h('Button', {
                ...
                on: {
                  click: () => {
                    this.$router.push({name: 'quiz-details', params: {quizID: params.row._id}})
                  }
                },
                style: {
                  padding: '2px 0'
    ...
    methods: {
      ...
      getQuizList () {
        ...
        api.getQuizList(offset, this.limit, this.query).then(res => {
          this.loadings.table = false
          this.total = res.data.data.total
          this.quizList = res.data.data.results
          if (this.isAuthenticated) {
            this.addStatusColumn(this.quizTableColumns, res.data.data.results)
          }
        }, res => {
          this.loadings.table = false
        })
      ...
      handleTagsVisible (value) {
        if (value) {
          ...
          this.quizTableColumns.splice(this.quizTableColumns.length - 1, 1)
        }
      },
      onReset () {
      ...
      pickone () {
        api.pickone().then(res => {
          ...
          this.$router.push({name: 'quiz-details', params: {quizID: res.data.data}})
        })
      }
    },
    computed: {
